...
impl MediaControls {
    pub fn new() -> Self {
        ...
        MediaControls {
            ...
            icon,
            pause_icon,
            artist: ScrollingTextWidget::new("".to_string(), &fonts::PIXELLARI, FONT_SIZE, Alignment::Center, SCROLL_WAIT, SCROLL_WAIT_END),
        ...
        match selected_session {
            Some(session) => {
                ...
                let icon_bounds = EmptyBounds::new()
                    .with_size(icon.size())
                    .with_y(0).center_hor_in(&canvas_bounds);
                ...
                let artist_bounds = EmptyBounds::new()
                    ...
                    .with_x(0).align_bottom(&canvas_bounds);
                ...
                let title_bounds = EmptyBounds::new()
                    .with_width(canvas.width as u32).with_height(self.title.size().height)
                    .with_x(0)
                    .center_ver_in(&EmptyBounds::new().between_ver(icon_bounds.pos.y + icon_bounds.size.height as i32, artist_bounds.pos.y));
